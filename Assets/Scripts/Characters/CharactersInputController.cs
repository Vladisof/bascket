...
namespace Characters
{
  public class CharactersInputController : MonoBehaviour
  {
    ...
    private static readonly int sThrowHash = Animator.StringToHash("Throw");
    private static readonly int sThrowStopHash = Animator.StringToHash("StopThrow");
    private static readonly int sThrowingHash = Animator.StringToHash("ThrowSpeed");
    ...
    private float _mThrowStart;
    private bool _mThrowing;
    ...
    private const float k_TrackToThrowAnimSpeedRatio = 0.6f;
    protected const float TRACK_SPEED_TO_SLIDE_ANIM_SPEED_RATIO = 0.9f;

    protected void Awake()
    {
      shootButton.onClick.AddListener(Throw);
      _mPremium = 0;
      _mCurrentLife = 0;
      _mSliding = false;
      _mSlideStart = 0.0f;
    ...
    protected void Update()
    {
      ...
      if (_mThrowing)
      {
        float correctThrowLength = slideLength * (1.0f + TracksManager.speedRatio);
        float ratio = (TracksManager.worldDistance - _mThrowStart) / correctThrowLength;
    ...
    private void Throw()
    {
      ...
      Characters.animator.ResetTrigger(sThrowStopHash);
      float correctThrowLength = throwLength * (1.0f + TracksManager.speedRatio);
      float animSpeed = k_TrackSpeedToJumpAnimSpeedRatio * (TracksManager.speed / correctThrowLength);
      Characters.animator.SetFloat(sThrowingHash, 1);
      Characters.animator.SetTrigger(sThrowHash);

      _mThrowStart = TracksManager.worldDistance;
      _mThrowing = true;
    }
    ...
    private void StopThrowing()
    {
      ...
      {
        shootBullet.Shoot();
        Characters.animator.SetTrigger(sThrowStopHash);
        _mThrowing = false;
      }
    }
    private void Slide()
